...
interface AppSettings {
  subject: string;
  animations: string[];
  prop_animations: string[];
  gear: string[];
  gear_animations: string[];
  ...
  hide_daz_instances: boolean;
  cache_db_size_threshold_gb: string;
  minimize_to_tray: boolean;
  start_on_startup: boolean;
  last_directories: Record<string, string>;
  [key: string]: unknown;
}
...
async function loadSettings(): Promise<void> {
  try {
    ...
    const settings: AppSettings = await ipcRenderer.invoke('load-settings');
...
function applySettings(settings: AppSettings): void {
  ...
  (document.getElementById('subject') as HTMLInputElement).value = settings.subject || '';
  (document.getElementById('animations') as HTMLTextAreaElement).value = (settings.animations || []).join('\n');
  (document.getElementById('prop-animations') as HTMLTextAreaElement).value = (settings.prop_animations || []).join('\n');
  (document.getElementById('gear') as HTMLTextAreaElement).value = (settings.gear || []).join('\n');
  (document.getElementById('gear-animations') as HTMLTextAreaElement).value = (settings.gear_animations || []).join('\n');
  (document.getElementById('output-dir') as HTMLInputElement).value = settings.output_directory || '';
  (document.getElementById('instances') as HTMLInputElement).value = settings.number_of_instances || '1';
  (document.getElementById('frame-rate') as HTMLInputElement).value = settings.frame_rate || '30';
  (document.getElementById('cache-threshold') as HTMLInputElement).value = settings.cache_db_size_threshold_gb || '10';
  console.log('Settings applied to UI');
}

function getSettings(): AppSettings {
  const renderShadowsEl = document.getElementById('render-shadows') as HTMLInputElement | null;
  const shutdownOnFinishEl = document.getElementById('shutdown-on-finish') as HTMLInputElement | null;
  
  return {
    subject: (document.getElementById('subject') as HTMLInputElement).value,
    animations: (document.getElementById('animations') as HTMLTextAreaElement).value.split('\n').filter(s => s.trim()),
    prop_animations: (document.getElementById('prop-animations') as HTMLTextAreaElement).value.split('\n').filter(s => s.trim()),
    gear: (document.getElementById('gear') as HTMLTextAreaElement).value.split('\n').filter(s => s.trim()),
    gear_animations: (document.getElementById('gear-animations') as HTMLTextAreaElement).value.split('\n').filter(s => s.trim()),
    output_directory: (document.getElementById('output-dir') as HTMLInputElement).value,
    export_destination: (currentSettings.export_destination as string) || '',
    number_of_instances: (document.getElementById('instances') as HTMLInputElement).value,
    frame_rate: (document.getElementById('frame-rate') as HTMLInputElement).value,
    cache_db_size_threshold_gb: (document.getElementById('cache-threshold') as HTMLInputElement).value,
    ...
    last_directories: (currentSettings.last_directories as Record<string, string>) || {}
  };
}
...
function clampNumberInputs(): void {
  const instances = document.getElementById('instances') as HTMLInputElement;
  const frameRate = document.getElementById('frame-rate') as HTMLInputElement;
  const cacheThreshold = document.getElementById('cache-threshold') as HTMLInputElement;
  
  // Clamp instances (min: 1, max: 99)
  if (instances.value) {
    const val = parseInt(instances.value);
    if (val < 1) {
      instances.value = '1';
    } else if (val > 99) {
      instances.value = '99';
  ...
  if (frameRate.value) {
    const val = parseInt(frameRate.value);
    if (val < 1) {
      frameRate.value = '1';
    } else if (val > 999) {
      frameRate.value = '999';
  ...
  if (cacheThreshold.value) {
    const val = parseInt(cacheThreshold.value);
    if (val < 5) {
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  inputs.forEach(el => {
    ...
    console.log(`Attached listeners to: ${inputEl.id || inputEl.name || inputEl.tagName}`);
  });
...
async function browseSubject(): Promise<void> {
  const result: string | undefined = await ipcRenderer.invoke('browse-file', { filters: [{ name: 'DAZ Files', extensions: ['duf'] }] });
  if (result) {
    (document.getElementById('subject') as HTMLInputElement).value = result;
    autoSave();
  }
}

async function browseAnimations(): Promise<void> {
  const result: string[] | undefined = await ipcRenderer.invoke('browse-files', { filters: [{ name: 'DAZ Files', extensions: ['duf'] }] });
  if (result && result.length) {
    const currentValue = (document.getElementById('animations') as HTMLTextAreaElement).value.trim();
    const newFiles = currentValue ? currentValue + '\n' + result.join('\n') : result.join('\n');
    (document.getElementById('animations') as HTMLTextAreaElement).value = newFiles;
    autoSave();
  }
}

async function browsePropAnimations(): Promise<void> {
  const result: string[] | undefined = await ipcRenderer.invoke('browse-files', { filters: [{ name: 'DAZ Files', extensions: ['duf'] }] });
  if (result && result.length) {
    const currentValue = (document.getElementById('prop-animations') as HTMLTextAreaElement).value.trim();
    const newFiles = currentValue ? currentValue + '\n' + result.join('\n') : result.join('\n');
    (document.getElementById('prop-animations') as HTMLTextAreaElement).value = newFiles;
    autoSave();
  }
}

async function browseGear(): Promise<void> {
  const result: string[] | undefined = await ipcRenderer.invoke('browse-files', { filters: [{ name: 'DAZ Files', extensions: ['duf'] }] });
  if (result && result.length) {
    const currentValue = (document.getElementById('gear') as HTMLTextAreaElement).value.trim();
    const newFiles = currentValue ? currentValue + '\n' + result.join('\n') : result.join('\n');
    (document.getElementById('gear') as HTMLTextAreaElement).value = newFiles;
    autoSave();
  }
}

async function browseGearAnimations(): Promise<void> {
  const result: string[] | undefined = await ipcRenderer.invoke('browse-files', { filters: [{ name: 'DAZ Files', extensions: ['duf'] }] });
  if (result && result.length) {
    const currentValue = (document.getElementById('gear-animations') as HTMLTextAreaElement).value.trim();
    const newFiles = currentValue ? currentValue + '\n' + result.join('\n') : result.join('\n');
    (document.getElementById('gear-animations') as HTMLTextAreaElement).value = newFiles;
    autoSave();
  }
}

async function browseOutputDir(): Promise<void> {
  const result: string | undefined = await ipcRenderer.invoke('browse-directory');
  if (result) {
    (document.getElementById('output-dir') as HTMLInputElement).value = result;
    autoSave();
  }
}
...
function clearAnimations(): void {
  (document.getElementById('animations') as HTMLTextAreaElement).value = '';
  autoSave();
}

function clearPropAnimations(): void {
  (document.getElementById('prop-animations') as HTMLTextAreaElement).value = '';
  autoSave();
}

function clearGear(): void {
  (document.getElementById('gear') as HTMLTextAreaElement).value = '';
  autoSave();
}

function clearGearAnimations(): void {
  (document.getElementById('gear-animations') as HTMLTextAreaElement).value = '';
  autoSave();
}
...
async function startRender(): Promise<void> {
  ...
  const result: RenderResult = await ipcRenderer.invoke('start-render', settings);
...
async function stopRender(): Promise<void> {
...
async function showOutputFolder(): Promise<void> {
...
async function showImageInFolder(): Promise<void> {
  if (currentImagePath) {
...
function clearAllFields(): void {
  ...
  (document.getElementById('gear') as HTMLTextAreaElement).value = '';
  (document.getElementById('gear-animations') as HTMLTextAreaElement).value = '';
  (document.getElementById('output-dir') as HTMLInputElement).value = '';
  autoSave();
}

function restoreDefaults(): void {
  (document.getElementById('instances') as HTMLInputElement).value = '1';
  (document.getElementById('frame-rate') as HTMLInputElement).value = '30';
  (document.getElementById('cache-threshold') as HTMLInputElement).value = '10';
  autoSave();
}
...
async function showSettings(): Promise<void> {
  ...
  const settings: AppSettings = await ipcRenderer.invoke('load-settings');
  (document.getElementById('minimize-to-tray') as HTMLInputElement).checked = settings.minimize_to_tray || false;
  (document.getElementById('start-on-startup') as HTMLInputElement).checked = settings.start_on_startup || false;
  (document.getElementById('hide-daz-instances') as HTMLInputElement).checked = settings.hide_daz_instances || false;
  (document.getElementById('render-shadows') as HTMLInputElement).checked = settings.render_shadows !== false;
  (document.getElementById('shutdown-on-finish') as HTMLInputElement).checked = settings.shutdown_on_finish !== false;
}
...
async function onSettingChange(key: string, value: boolean): Promise<void> {
  ...
  if (key === 'start_on_startup') {
    await ipcRenderer.invoke('manage-windows-startup', value);
  }
}

async function exitOverlord(): Promise<void> {
  window.close();
}
...
async function showOverlordLog(): Promise<void> {
  const appData = process.env.APPDATA || path.join(os.homedir(), 'AppData', 'Roaming');
  const logPath = path.join(appData, 'Overlord', 'log.txt');
  shell.openPath(logPath);
}

async function showIrayServerLog(): Promise<void> {
  const appData = process.env.APPDATA || path.join(os.homedir(), 'AppData', 'Roaming');
...
async function showDazStudioLog(): Promise<void> {
  const appData = process.env.APPDATA || path.join(os.homedir(), 'AppData', 'Roaming');
...
function showIrayServerInterface(): void {
...
async function showAbout(): Promise<void> {
  const version: string = await ipcRenderer.invoke('get-version');
...
async function fetchPatchNotes(): Promise<void> {
  ...
  try {
    ...
    const tagName: string = releaseData.tag_name || 'Unknown';
    const releaseName: string = releaseData.name || 'Unknown Release';
    const body: string = releaseData.body || 'No release notes available.';
    const publishedAt: string = releaseData.published_at || '';
...
function processInlineMarkdown(text: string): string {
  ...
  result = result.replace(/(https?:\/\/[^\s<]+)/g, '<a href="#" onclick="openUrl(\'$1\'); return false;" style="color: #0078d4; text-decoration: none;">$1</a>');
...
ipcRenderer.on('image-updated', (_event: Electron.IpcRendererEvent, imageData: ImageData) => {
  ...
  try {
    ...
    const img = new Image();
    img.onload = function() {
      document.getElementById('info-resolution')!.textContent = `${img.width} × ${img.height}`;
    };
...
ipcRenderer.on('render-progress', (_event: Electron.IpcRendererEvent, progressData: RenderProgress) => {
  // Update progress bar
  const progressFill = document.getElementById('progress-fill')!;
  progressFill.style.width = progressData.progressPercent.toFixed(1) + '%';
  
  // Update output details
  document.getElementById('session-images')!.textContent = String(progressData.sessionCount);
  document.getElementById('total-images')!.textContent = String(progressData.renderedCount);
  document.getElementById('images-remaining')!.textContent = String(progressData.remaining);
...
function showConstructExporter(): void {
  const modal = document.createElement('div');
  ...
  const exportDest = (currentSettings.export_destination as string) || outputDir;
  (document.getElementById('exporter-destination-path') as HTMLInputElement).value = exportDest;
...
async function browseExportDestination(): Promise<void> {
  try {
    const directory: string | undefined = await ipcRenderer.invoke('browse-directory');
    if (directory) {
      (document.getElementById('exporter-destination-path') as HTMLInputElement).value = directory;
      
      // Save the export destination to settings
      currentSettings.export_destination = directory;
      if (currentSettings.last_directories) {
        (currentSettings.last_directories as Record<string, string>).export_destination = path.dirname(directory);
      }
...
function closeConstructExporter(): void {
...
async function runConstructExporter(): Promise<void> {
  const yesBtn = document.getElementById('exporter-yes-btn') as HTMLButtonElement;
  ...
  const destinationPath = (document.getElementById('exporter-destination-path') as HTMLInputElement).value;
  ...
  try {
    ...
    const outputListener = (_event: Electron.IpcRendererEvent, text: string): void => {
    ...
    await ipcRenderer.invoke('run-construct-exporter', destinationPath);
    ...
    const err = error as Error;
    console.error('Construct Exporter error:', err);
