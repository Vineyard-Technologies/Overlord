 ...
 */
async function createZipArchive(baseName: string, files: FileInfo[], outputPath: string): Promise<string | null> {
 ...
 */
async function organizeFiles(sourceDir: string, outputBaseDir: string, maxConcurrent: number | null = null): Promise<void> {
  ...
  const fileGroups: Record<string, FileInfo[]> = {};
  ...
  const zipTasks: ZipTask[] = Object.entries(fileGroups).map(([baseName, files]) => ({
    baseName,
    files,
    outputPath
  }));
...
export { organizeFiles, parseFilename, createZipArchive };
