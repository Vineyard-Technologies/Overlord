...
const execPromise = util.promisify(child_process_2.exec);
// ============================================================================
...
function getDisplayVersion() {
    return electron_1.app.isPackaged ? APP_VERSION : 'dev';
}
...
async function startRender(settings) {
    ...
    initialTotalImages = calculateTotalImages(settings.subject, settings.animations, settings.gear, settings.render_shadows);
    ...
    for (let i = 0; i < numInstances; i++) {
        ...
        (0, child_process_1.spawn)(dazExecutablePath, command, { detached: true, stdio: 'ignore' });
        if (i < numInstances - 1) {
...
electron_1.ipcMain.handle('manage-windows-startup', async (_event, enable) => {
    try {
...
electron_1.ipcMain.handle('browse-file', async (_event, options) => {
...
electron_1.ipcMain.handle('browse-files', async (_event, options) => {
    const result = await electron_1.dialog.showOpenDialog(mainWindow, {
...
electron_1.ipcMain.handle('browse-directory', async () => {
    const result = await electron_1.dialog.showOpenDialog(mainWindow, {
        properties: ['openDirectory']
    });
    return result.filePaths[0];
});
electron_1.ipcMain.handle('start-render', async (_event, settings) => {
    try {
...
electron_1.ipcMain.handle('stop-render', async () => {
    return await stopRender();
});
electron_1.ipcMain.handle('copy-to-clipboard', (_event, text) => {
    electron_1.clipboard.writeText(text);
});
electron_1.ipcMain.handle('show-folder', async (_event, folderPath) => {
    try {
        ...
        if (fs.existsSync(folderPath)) {
            ...
            electron_1.dialog.showErrorBox('Folder Not Found', `The folder does not exist:\n${folderPath}`);
        }
    }
    catch (error) {
        const err = error;
        console.error('Error opening folder:', err);
        electron_1.dialog.showErrorBox('Error', `Failed to open folder: ${err.message}`);
    }
});
electron_1.ipcMain.handle('open-file', async (_event, filePath) => {
    try {
        ...
        if (fs.existsSync(filePath)) {
            ...
            electron_1.dialog.showErrorBox('File Not Found', `The file does not exist:\n${filePath}`);
        }
    }
    catch (error) {
        const err = error;
        console.error('Error opening file:', err);
        electron_1.dialog.showErrorBox('Error', `Failed to open file: ${err.message}`);
    }
});
electron_1.ipcMain.handle('get-version', () => {
    return getDisplayVersion();
});
electron_1.ipcMain.handle('run-construct-exporter', async (_event, destinationPath) => {
    return new Promise((resolve, reject) => {
        try {
            const settings = settingsManager.loadSettings();
            ...
            const nodeProcess = (0, child_process_1.spawn)(command, [], {
...
process.on('uncaughtException', (error) => {
    ...
    electron_1.dialog.showErrorBox('Fatal Error', `An unexpected error occurred:\n\n${error.message}\n\nThe application will now exit.`);
    electron_1.app.quit();
});
    ...
    try {
        ...
        setTimeout(() => {
            try {
                ...
                electron_1.dialog.showErrorBox('Startup Error', `Failed to create application window:\n\n${err.message}`);
            }
        }, 500);
        ...
        electron_1.dialog.showErrorBox('Startup Error', `Failed to initialize application:\n\n${err.message}`);
        electron_1.app.quit();
    }
}).catch((error) => {
    console.error('Error in app.whenReady:', error);
    electron_1.dialog.showErrorBox('Fatal Error', `Application failed to start:\n\n${error.message}`);
    electron_1.app.quit();
});
